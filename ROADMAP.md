...
- Commits landed so far: 1/6 design note + ROADMAP flip (#4/#5 `[ ]` → `[~]`), 2/6 fixture extension, 3/6 `compute_trade_alpha_postfile` primitive, 4/6 composite cutover + dry run against the cached universe (30% top-30 churn — see design note's "Dry run" section).
- Next commits: leaderboard schema → backtest module.
...
_In progress — plan approved session 3 (2026-04-23). Shared design note: `design/postfile-alpha-and-backtest.md`. 6-commit sequence; 4/6 landed (design note + ROADMAP flip, fixture extension, `compute_trade_alpha_postfile` primitive, composite cutover + dry run). Remaining: leaderboard schema, backtest module (#5)._
